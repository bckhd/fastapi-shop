...
class CartService:
    ...
    def add_cart_item(self, cart_data: Dict[int, int], item: CartItemCreate) -> Dict[int, int]:
    ...
    def update_cart_item(self, cart_data: Dict[int, int], item: CartItemUpdate) -> Dict[int, int]:
        ...
        if product_id not in cart_data:
            raise HTTPException(status_code=404, detail=f'Product with id {product_id} not found in cart')
